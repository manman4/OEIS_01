def step_states(states, active_masks, valid_bits, state_count)
  return [[], []] if active_masks.empty?

  degree = states[active_masks[0]].length - 1
  ...
  active_masks.each{|mask|
    ...
    if stay.nil?
      stay = next_states[shifted] = Array.new(degree + 2, 0)
      seen[shifted] = true
    ...
    valid_bits.each{|bit|
      ...
      if add_rook.nil?
        add_rook = next_states[shifted_with_rook] = Array.new(degree + 2, 0)
...
def evaluate_states(states, active_masks, n, factorials)
...
def direct_count(n, k, factorials)
  ...
  1.upto(n){|row|
    ...
    states, active_masks = step_states(states, active_masks, valid_bits, state_count)
  }

  evaluate_states(states, active_masks, n, factorials)
end
...
def sequence(limit, k)
  ...
  while n <= limit
    ...
    tail_valid_bits.each{|valid_bits|
      tail_states, tail_active_masks = step_states(tail_states, tail_active_masks, valid_bits, state_count)
    }
    result << evaluate_states(tail_states, tail_active_masks, n, factorials)

    n += 1
    break if n > limit

    states, active_masks = step_states(states, active_masks, full_valid_bits, state_count)
  end
