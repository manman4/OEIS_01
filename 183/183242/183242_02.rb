...
def rook_step(states, active_masks, valid_bits, state_count)
...
def aggregate_rook(states, active_masks, n)
...
def rook_polynomial(n, k)
  ...
  1.upto(n){|row|
    ...
    states, active_masks = rook_step(states, active_masks, valid_bits, state_count)
  }

  aggregate_rook(states, active_masks, n)
end
...
def sequence(limit, k)
  ...
  while n <= limit
    ...
    tail_valid_bits.each{|valid_bits|
      tail_states, tail_active_masks = rook_step(tail_states, tail_active_masks, valid_bits, state_count)
    }
    rook = aggregate_rook(tail_states, tail_active_masks, n)
    result << count_from_rook(rook, facts)

    n += 1
    break if n > limit

    states, active_masks = rook_step(states, active_masks, full_valid_bits, state_count)
  end
