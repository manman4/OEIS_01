/*
 * A179962 and the general fixed-k family -- certified for 2<=k<=8.
 *
 * IMPORTANT STATUS
 * ----------------
 ...
 *   (2) For each k=2,...,8, an exact finite-window certificate proves that
 *       the recurrence C_k used below holds from its stated sharp boundary.
 *       Thus every value generated by this program is proved equal to the
 *       combinatorial definition, not merely fitted or extrapolated.
 *       The k=8 certificate was completed with a C/GMP implementation of the
 *       insertion DP (diag.c): 13629 consecutive residual zeros through
 *       n=13640 establish C_8 for all n>=12, and exact values close the
 *       remaining gap down to the sharp boundary n>=9.  It was reproduced
 *       independently on a second machine and compiler.
 *   (3) For k>=9 the identification of C_k remains open, as does a uniform
 *       proof for arbitrary k.  C_9 agrees with every checkable point
 *       (n=12..16) but its certificate has not been run: the window scales
 *       with the live-state count (about 7*10^4 for k=9), where memory, not
 ...
 * needed for the fixed-k certificates for k=2,...,8.
 *
 * NOTATION
 * --------
 ...
 * law below.
 *
 * GENERAL SPECTRAL LAW -- CONJECTURAL; FIXED k<=8 CERTIFIED
 * ---------------------------------------------------------
 ...
 * this recurrence and its start point separately for every k=2,...,8.
 * For k>=9 and unrestricted k it remains conjectural.  Examples are
 *
 *   k=3: A(n)=4*A(n-1)-4*A(n-2),                    n>=2;
 *   k=4: A(n)=7*A(n-1)-12*A(n-2),                  n>=3;
 *   k=5: A(n)=16*A(n-1)-84*A(n-2)+144*A(n-3),     n>=5;
 *   k=8: A(n)=50*A(n-1)-913*A(n-2)+7164*A(n-3)
 *              -20160*A(n-4),                      n>=9.
 *
 ...
 * establish the already certified cases k=2,...,8.  A cheaper certificate
 * would follow from a better degree bound: Lemma 10.1 only needs an upper
 * bound on the order of some recurrence satisfied by A_k, so replacing the
 * live-state count by the Krylov dimension -- find a candidate P by linear
 * algebra mod p, then verify P(Lambda)h=0 exactly over Z -- would shorten
 * every window, k=9 included.
 *
 ...
 *
 * For k=2,...,8, the finite-window computation is part of the proof because
 ...
 * implementation matches that proof.  For k>=9 they remain finite evidence.
 *
 ...
 *
 * agreeing with (C6).  That was a prediction check made before the k=8
 * certificate existed; C_8 is now proved, and the insertion DP reproduces
 * the same value in well under a second.  The embedded k=8 list has been
 * extended to n=13 with values from that DP, so --check now compares the
 * recurrence against independently counted values past the boundary.
 *
 * The k=8 certificate itself ran as follows.  The live-state dimension is
 * 13628, so 13629 residual zeros suffice (Lemma 10.1 needs 13628; the
 * implementation takes the safer D+1).  Generating n up to 13640 produced an
 * unbroken run of 13629 zeros, certifying C_8 for all n>=12.  The remaining
 * gap was closed by exact values: the recurrence holds at n=9,10,11 and
 * fails at n=8 and n=7 (residuals 173654713728 and -11145216), so the sharp
 * boundary is n>=9=d+max(0,k-3), exactly as (C6) predicts.  The two small
 * values needed for those residuals, A_8(3)=99133496 and A_8(4)=1572313392,
 * were recomputed by the independent inclusion-exclusion DP and agree with
 * the seeds above.  Timings: 1250 s single-threaded, 1674 s on eight threads
 * (the per-state GMP work is too fine-grained for the thread overhead to pay
 * off), reproduced on two different machines and compilers.
 ...
 * stationary seeds n=6..10, so k=9 is deliberately not exposed.  Note that
 * the insertion DP computes A_9(n) unconditionally (its correctness does not
 * depend on any recurrence), so the missing seeds are obtainable; what is
 * missing for k=9 is the certificate for C_9, not the values.
 *
 ...
 *
 * References:
 *   files/proved_results.md (finite-state proof and k<=8 certificates).
...
static const char *const m2_k8_known[] = {
    "40320", "479306", "6564318", "99133496", "1572313392",
    "25415753280", "412583686272", "6690112284672",
    "108370981896192", "1753844521231872", "28359796882516992",
    "458226846886482432", "7398630945664008192",
    "119382515394159140352"
};
